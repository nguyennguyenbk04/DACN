...
const SUMMARY_MODELS = [
  { value: 'led',                           label: 'LED',           hint: 'Local model, no API needed' },
  { value: 'groq/llama-3.1-8b-instant',    label: 'Llama 3.1 8B',  hint: 'Groq — Fast' },
  { value: 'groq/llama-3.3-70b-versatile', label: 'Llama 3.3 70B', hint: 'Groq — Higher quality' },
];
...
export default function TranscriptViewer({ videoId, onClose }) {
  ...
  const [summaryModel, setSummaryModel]     = useState('led');
  ...
  async function handleGenerateSummary() {
    ...
    try {
      const data = await api.generateSummary(videoId, { length: summaryLength, model: summaryModel });
      setSummary(data.summary);
  ...
  return (
    <div className="h-full flex flex-col">
      {/* Tab bar */}
      <div className="flex-shrink-0 flex items-center border-b border-gray-200 bg-white px-4">
        {TABS.map(tab => (
          <button
            key={tab.id}
        ...
        {isTranscribed && (
          <div className="ml-auto flex items-center gap-1">
            <button onClick={copyTranscript} className="flex items-center gap-1.5 px-3 py-1.5 text-xs text-gray-500 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition">
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" />
              </svg>
              Copy
            </button>
            <button onClick={exportSRT} className="flex items-center gap-1.5 px-3 py-1.5 text-xs text-gray-500 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition">
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
              </svg>
              .srt
            </button>
            <button onClick={exportTXT} className="flex items-center gap-1.5 px-3 py-1.5 text-xs text-gray-500 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition">
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
              </svg>
              .txt
      ...
      <div className="flex-1 overflow-y-auto">
          ...
          <div className="p-5 space-y-4">
            {jobDetails?.videoPresignedUrl || jobDetails?.youtubeUrl ? (
              ...
              <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <p className="text-sm text-gray-400">No video source available</p>
              </div>
            )}

            {/* Transcript with timestamps under video */}
            {isTranscribed && (
              <div className="border border-gray-200 rounded-xl overflow-hidden">
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest px-4 py-2 border-b border-gray-200 bg-gray-50">Transcript</p>
                <div className="max-h-64 overflow-y-auto divide-y divide-gray-100">
                  {(transcript.segments || []).map((seg, i) => {
                    const isActive = currentTime >= seg.start && currentTime <= seg.end;
                    return (
                      <div key={i} className={`flex gap-3 px-4 py-2 transition-colors ${isActive ? 'bg-indigo-50' : 'hover:bg-gray-50'}`}>
                        <button
                          onClick={() => videoPlayerRef.current?.seekTo(seg.start)}
                          className={`shrink-0 font-mono text-[10px] px-1.5 py-0.5 rounded mt-0.5 transition-colors ${
                            isActive ? 'bg-indigo-100 text-indigo-600' : 'bg-gray-100 text-gray-400 hover:text-indigo-500'
                          }`}
                        >
                          {fmt(seg.start)}
                        </button>
                        <p className={`text-xs leading-relaxed flex-1 ${isActive ? 'text-gray-900 font-medium' : 'text-gray-600'}`}>
                          {seg.text}
                        </p>
                      </div>
                    );
        ...
        {activeTab === 'transcript' && (
          <div>
            ...
            {(status === 'ready' || status === 'failed') && (
              <div className="flex flex-col items-center justify-center py-16 px-8 text-center gap-6">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-400">Select a Whisper model and click Transcribe to generate the transcript.</p>
                </div>

                {/* Model selector */}
                <div className="w-full max-w-sm space-y-3">
                  <label className="text-xs font-bold text-gray-400 uppercase tracking-wider block text-left">
                    Whisper Model
                  </label>
                      ...
                      <button
                        ...
                        className={`flex items-center justify-between px-4 py-2.5 rounded-xl border text-sm transition ${
                          whisperModel === m.value
                            ? 'border-indigo-500 bg-indigo-600/15 text-gray-900'
                            : 'border-gray-200 bg-gray-100 text-gray-500 hover:border-[#3a3a5e]'
                        }`}
                      >
                  ...
                  <button
                    ...
                    className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-gray-200 disabled:text-gray-400 disabled:cursor-not-allowed text-white font-semibold py-2.5 rounded-xl transition flex items-center justify-center gap-2 mt-2"
                  >
            ...
            {isProcessing && (
              <div className="flex flex-col items-center justify-center py-20 gap-5">
                ...
                <div className="text-center">
                  ...
                  <p className="text-sm text-gray-400">This may take a few minutes depending on the model and file length.</p>
                </div>
              </div>
            )}
            ...
            {isTranscribed && (
              <div>
                {/* Metadata strip */}
                <div className="flex items-center gap-4 px-5 py-3 border-b border-gray-200 text-xs text-gray-400">
                  <span>Language: <span className="text-gray-500">{transcript.language || '—'}</span></span>
                  <span>Words: <span className="text-gray-500">{transcript.fullText?.split(/\s+/).length?.toLocaleString() ?? 0}</span></span>
                  <span>Characters: <span className="text-gray-500">{transcript.fullText?.length?.toLocaleString() ?? 0}</span></span>
                </div>
                ...
                <div className="px-5 pb-5 pt-2 border-t border-gray-200 mt-2">
                  <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Translate</p>
                  <div className="flex items-center gap-2">
                    <select
                      value={transcriptLang}
                      onChange={e => setTranscriptLang(e.target.value)}
                      disabled={translatingTranscript}
                      className="flex-1 bg-gray-100 border border-gray-200 rounded-lg px-3 py-1.5 text-xs text-gray-900 focus:outline-none focus:border-indigo-500 disabled:opacity-50"
                    >
                  ...
                  {translatedTranscript && (
                    <div className="mt-3 p-4 bg-gray-100 border border-gray-200 rounded-xl text-sm text-gray-700 leading-relaxed whitespace-pre-wrap max-h-60 overflow-y-auto">
        ...
        {activeTab === 'summary' && (
          <div className="p-5 space-y-5">
            {!isTranscribed && (
              <div className="p-6 bg-gray-100 border border-dashed border-gray-200 rounded-xl text-sm text-gray-400 text-center">
              ...
              <>
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  <button
                    ...
                    className="bg-indigo-600 text-white py-1.5 px-4 rounded-lg hover:bg-indigo-700 disabled:bg-gray-200 disabled:text-gray-400 disabled:cursor-not-allowed text-xs font-medium transition"
                  >
                ...
                <div className="flex flex-wrap items-center gap-3">
                  <select
                    ...
                    disabled={summarizing}
                    className="bg-white border border-gray-200 rounded-lg px-3 py-1.5 text-xs text-gray-900 focus:outline-none focus:border-indigo-500 disabled:opacity-50 cursor-pointer"
                  >
                      ...
                      {SUMMARY_LENGTHS.map(opt => (
                        <button
                          ...
                          className={`px-3 py-1 rounded-md text-xs font-medium transition ${
                            summaryLength === opt.value ? 'bg-indigo-600 text-white' : 'text-gray-400 hover:text-gray-500'
                          } disabled:opacity-50`}
                        >
                          {opt.label}
                        </button>
                ...
                {summary ? (
                  <div className="p-4 bg-gray-100 border border-gray-200 rounded-xl text-sm text-gray-700 leading-relaxed">
                    {summary}
                  </div>
                ) : !summarizing && (
                  <div className="p-8 bg-gray-100 border border-dashed border-gray-200 rounded-xl text-sm text-gray-400 text-center">
                ...
                {summary && (
                  <div className="space-y-3">
                    <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Translate</p>
                    <div className="flex items-center gap-2">
                      <select
                        value={summaryLang}
          ...
          <div className="p-5 space-y-5">
            ...
            {isTranscribed && (
              <>
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-semibold text-gray-900">Multiple Choice Quiz</span>
                    {savedQuizMeta && (
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="bg-amber-600 text-white py-1.5 px-4 rounded-lg hover:bg-amber-700 disabled:bg-gray-200 disabled:text-gray-400 disabled:cursor-not-allowed text-xs font-medium transition"
                    >
          ...
          <div className="p-5 space-y-6">
            ...
            <section>
              <div className="flex items-center gap-2 mb-1">
                <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Evaluation Metrics</p>
                <span className="text-[10px] text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">thesis</span>
              </div>
              <p className="text-xs text-gray-400 mb-4">Compare model output against a human-written reference.</p>
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-medium transition border ${
                      activeEval === key
                        ...
                        : 'bg-gray-100 text-gray-500 border-gray-200 hover:border-indigo-500'
                    }`}
                  >
                    {label}
                  </button>
              ...
              {activeEval && (
                <div className="space-y-3">
                  <textarea
                    ...
                    className="w-full bg-gray-100 border border-gray-200 rounded-lg px-3 py-2 text-xs text-gray-900 placeholder-[#555570] focus:outline-none focus:border-indigo-500 resize-none"
                  />
                  <button
                    onClick={handleEvaluate}
                    disabled={evaluating || !evalReference.trim()}
                    className="bg-gray-200 text-gray-900 text-xs px-4 py-2 rounded-lg hover:bg-[#3a3a5e] disabled:opacity-40 transition"
                  >
                  ...
                  {evalResult && (
                    <div className="p-4 bg-gray-100 border border-gray-200 rounded-xl">
                      {activeEval === 'wer' ? (
                        <div>
                          <p className="text-xs font-semibold text-gray-500 mb-1">Word Error Rate</p>
                        ...
                        <div className="space-y-2">
                          <p className="text-xs font-semibold text-gray-500 mb-3">ROUGE Scores (F1)</p>
                          {['rouge1', 'rouge2', 'rougeL'].map(key => (
                            <div key={key} className="flex items-center gap-3">
                              <span className="text-xs font-mono w-16 text-gray-400">{key}</span>
