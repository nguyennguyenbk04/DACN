...
export default function FileUpload({ onUploadComplete }) {
  ...
  return (
    <div className="space-y-3">
      {/* Tab switcher */}
      <div className="flex bg-[#0d0d14] rounded-lg p-0.5">
        {[{ id: 'file', label: 'File' }, { id: 'youtube', label: 'YouTube' }].map(t => (
          <button
            key={t.id}
            onClick={() => { setTab(t.id); reset(); }}
            className={`flex-1 py-1.5 text-xs font-medium rounded-md transition ${
              tab === t.id ? 'bg-[#2a2a3e] text-[#e8e8f0]' : 'text-[#555570] hover:text-[#8888a8]'
            }`}
          >
            {t.label}
          </button>
      ...
      {tab === 'file' ? (
        <div
          className={`relative rounded-xl border-2 border-dashed transition-colors ${
            dragActive ? 'border-indigo-500 bg-indigo-500/5' : 'border-[#2a2a3e] hover:border-[#3a3a5e]'
          } ${file ? 'p-3' : 'py-7 px-3'}`}
        ...
        >
          ...
          {!file ? (
            <label htmlFor="file-upload" className="cursor-pointer flex flex-col items-center gap-2">
              <svg className="w-8 h-8 text-[#555570]" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
              ...
              <div className="text-center">
                <p className="text-xs text-[#8888a8]">Drop video or audio</p>
                <p className="text-[10px] text-[#555570] mt-0.5">MP4, MP3, WAV, WEBM…</p>
              </div>
            </label>
          ) : (
            <div className="flex items-center gap-2">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs font-medium text-[#e8e8f0] truncate">{file.name}</p>
                <p className="text-[10px] text-[#555570]">{(file.size / 1024 / 1024).toFixed(1)} MB</p>
              </div>
              {!uploading && (
                <button onClick={() => setFile(null)} className="p-1 rounded hover:bg-[#2a2a3e] text-[#555570] hover:text-[#e8e8f0] transition">
        ...
        <input
          ...
          disabled={uploading}
          className="w-full bg-[#0d0d14] border border-[#2a2a3e] rounded-lg px-3 py-2 text-xs text-[#e8e8f0] placeholder-[#555570] focus:outline-none focus:border-indigo-500 disabled:opacity-50"
        />
      )}
      ...
      <button
        ...
        className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-[#2a2a3e] disabled:text-[#555570] disabled:cursor-not-allowed text-white text-xs font-semibold py-2 rounded-lg transition flex items-center justify-center gap-1.5"
      >
        {uploading ? (
          <>
