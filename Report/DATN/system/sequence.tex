...
After the transcription is complete, the user can request a summary. As shown in Figure \ref{fig:uc2_seq}, the Summarization Service retrieves the full transcript from MongoDB, processes it through the LED (Longformer Encoder-Decoder) model to generate an abstractive summary, and returns the result to the user.
...
Figure \ref{fig:uc3_seq} depicts the flow for generating assessments. The Quiz Service passes the transcript through a two-stage pipeline — a question generation model followed by a distractor generation model — to produce a set of multiple-choice questions. These questions are then stored in the MySQL database for later retrieval.
